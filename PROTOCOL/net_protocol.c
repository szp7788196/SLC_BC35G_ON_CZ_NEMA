...
u16 MakeLogin_out_heartbeatFrame(u8 afn,u8 fn,u8 *outbuf)
{
	...
	else if(afn == 0x0E && (fn == 1 || fn == 2))		//事件上报
	{
		...
		if((EventReport & (1 << ((long long)event_lable[ecx - ec_flag] - 1))) == 0x0000000000000000)
		{
			res = 0;
		...
		if((EventImportant & (1 << ((long long)event_lable[ecx - ec_flag] - 1))) == 0x0000000000000000)		//一般事件	
		{
			fn_really = 2;	//一般事件Fn
		}
