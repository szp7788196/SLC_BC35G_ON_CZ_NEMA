	...
	else if(MyStrstr(inbuf, (u8 *)"=\"", inbuf_len, 2) != 0xFFFF)
	{
		...
		if(get_str1(inbuf, "\"", 1, "\"", 2, &content[1]))
		{
			...
			if(content[0] < 33)
			{
				...
				memcpy(&HoldReg[SERVER_DOMAIN_ADD],content,SERVER_DOMAIN_LEN - 2);
	...
	else if(MyStrstr(inbuf, (u8 *)"=\"", inbuf_len, 2) != 0xFFFF)
	{
		...
		if(get_str1(inbuf, "\"", 1, "\"", 2, &content[1]))
		{
			...
			if(content[0] < 16)
			{
				...
				memcpy(&HoldReg[SERVER_IP_ADD],content,SERVER_IP_LEN - 2);
	...
	else if(MyStrstr(inbuf, (u8 *)"=\"", inbuf_len, 2) != 0xFFFF)
	{
		...
		if(get_str1(inbuf, "\"", 1, "\"", 2, &content[1]))
		{
			...
			if(content[0] < 6)
			{
				...
				memcpy(&HoldReg[SERVER_PORT_ADD],content,SERVER_PORT_LEN - 2);
