...
//					temp1 ++;	//加上这句会出错，闰年的12月31日会变成下一年的1月1日
	...
	CalendarClock[0] = ((calendar.sec / 10) << 4) + ((calendar.sec % 10) & 0x0F);		//秒
	CalendarClock[1] = ((calendar.min / 10) << 4) + ((calendar.min % 10) & 0x0F);		//分
	CalendarClock[2] = ((calendar.hour / 10) << 4) + ((calendar.hour % 10) & 0x0F);		//时
	CalendarClock[3] = ((calendar.w_date / 10) << 4) + ((calendar.w_date % 10) & 0x0F);	//日
	CalendarClock[4] = (calendar.week << 5) + (((calendar.w_month / 10) << 4) & 0x10) + ((calendar.w_month % 10) & 0x0F);	//星期 月
	CalendarClock[5] = (((calendar.w_year - 2000) / 10) << 4) + (((calendar.w_year - 2000) % 10) & 0x0F);	//日
